    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            Action::NavigateToEpicDetail { epic_id } => {
                ...
                self.pages.push( Box::new(EpicDetail { epic_id: epic_id, db: self.db.clone() }))
            }
            Action::NavigateToStoryDetail { epic_id, story_id } => {
                // create a new StoryDetail instance and add it to the pages vector
                self.pages.push( Box::new( StoryDetail {epic_id: epic_id, story_id: story_id, db: self.db.clone() }))
            }
            ...
            Action::CreateEpic => {
                ...
                let e = (self.prompts.create_epic)();
                self.db.create_epic(e).with_context(|| anyhow!("Error creating epic"))?;
            }
            Action::UpdateEpicStatus { epic_id } => {
                ...
                if let Some(status) = (self.prompts.update_status)() {
                    self.db.update_epic_status(epic_id, status).with_context(|| anyhow!("Error updating epic status"))?;
                }
            }
            Action::DeleteEpic { epic_id } => {
                // prompt the user to delete the epic and persist it in the database
                if (self.prompts.delete_epic)() {
                    self.db.delete_epic(epic_id).with_context(|| anyhow!("Error deleting epic"))?;
            ...
            Action::CreateStory { epic_id } => {
                ...
                self.db.create_story(story, epic_id).with_context(|| anyhow!("Error creating story"))?;
            }
            Action::UpdateStoryStatus { story_id } => {
                // prompt the user to update status and persist it in the database
                if let Some(status) = (self.prompts.update_status)() {
                    self.db.update_story_status(story_id, status).with_context(|| anyhow!("Error updating story status"))?;
                }
            }
            Action::DeleteStory { epic_id, story_id } => {
                ...
                if (self.prompts.delete_story)() {
                    self.db.delete_story(epic_id, story_id).with_context(|| anyhow!("Error deleting story"))?;
